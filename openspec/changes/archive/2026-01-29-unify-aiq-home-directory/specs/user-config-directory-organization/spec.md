...
- **THEN** system creates `~/.aiq/` with subdirectories: `config/`, `sessions/`, `skills/`, `tools/`, `prompts/`, `bin/`

#### Scenario: Config files location
- **WHEN** system saves configuration files
- **THEN** LLM config is saved to `~/.aiq/config/config.yaml` and data sources config is saved to `~/.aiq/config/sources.yaml`

#### Scenario: Session files location
- **WHEN** system saves session files
- **THEN** session files are saved to `~/.aiq/sessions/session_<timestamp>.json`

#### Scenario: Skills directory location
- **WHEN** system loads Skills
- **THEN** Skills are loaded from `~/.aiq/skills/<skill-name>/SKILL.md`

#### Scenario: Binary directory location
- **WHEN** aiq binary is installed via install script
- **THEN** binary is located at `~/.aiq/bin/aiq`
...
- **THEN** system resolves path to `~/.aiq/config/config.yaml`

#### Scenario: Get sources file path
- **WHEN** system needs to access data sources configuration
- **THEN** system resolves path to `~/.aiq/config/sources.yaml`

#### Scenario: Get sessions directory path
- **WHEN** system needs to save or load session files
- **THEN** system resolves path to `~/.aiq/sessions/`

#### Scenario: Get skills directory path
- **WHEN** system needs to load Skills
- **THEN** system resolves path to `~/.aiq/skills/`

#### Scenario: Get bin directory path
- **WHEN** system needs to reference binary location
- **THEN** system resolves path to `~/.aiq/bin/`
...
- **WHEN** application starts and `~/.aiq/` does not exist
- **THEN** system creates all required subdirectories (`config/`, `sessions/`, `skills/`, `tools/`, `prompts/`, `bin/`)
