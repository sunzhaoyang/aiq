...
- **THEN** system downloads binary from GitHub Releases (no timeout, user can Ctrl+C if slow)
...
- **WHEN** installation completes
- **THEN** system verifies that binary exists and is executable, displays version and success message
...
- **THEN** system queries GitHub Releases API using PowerShell to get the latest release tag
...
- **WHEN** system has determined version and architecture
- **THEN** system downloads `aiq-windows-amd64.exe` from GitHub Releases using PowerShell
