# AIQ

<div align="center">

**AIQ** - 一个将自然语言转换为 SQL 查询的智能 SQL 客户端

[![Go Version](https://img.shields.io/badge/go-1.21+-00ADD8?style=flat-square&logo=go)](https://golang.org)
[![License](https://img.shields.io/badge/license-Apache%202.0-blue.svg?style=flat-square)](LICENSE)

*用自然语言提问，获得精确的 SQL 查询，将结果可视化为精美的图表*

</div>

---

## 📖 简介

AIQ (AI Query) 是一个智能 SQL 客户端，通过自然语言与数据库交互。无需编写 SQL，只需用自然语言提问，AIQ 会自动生成 SQL 查询并执行，还能将结果可视化为精美的图表。

### ✨ 核心特性

- 🗣️ **自然语言查询** - 用中文或英文提问，自动生成 SQL
- 📊 **图表可视化** - 支持柱状图、折线图、饼图、散点图
- 🔌 **多数据库支持** - MySQL、PostgreSQL、SeekDB
- 🎨 **美观的 CLI 界面** - 流畅的交互体验和彩色输出
- ⚙️ **简单配置** - 引导式配置向导
- 🔒 **本地存储** - 配置和连接信息安全存储在本地

## 🚀 快速开始

### 安装

#### 从源码构建

```bash
# 克隆仓库
git clone https://github.com/aiq/aiq.git
cd aiq

# 构建二进制文件
go build -o aiq cmd/aiq/main.go

# 安装到系统路径（可选）
sudo mv aiq /usr/local/bin/
```

### 首次使用

1. **启动 AIQ**
   ```bash
   aiq
   ```

2. **配置 LLM**
   - 首次运行会启动配置向导
   - 输入 LLM API URL（如：`https://api.openai.com/v1`）
   - 输入 API Key
   - 输入模型名称（如：`gpt-4`）
   - 配置保存在 `~/.aiqconfig/config.yaml`

3. **添加数据源**
   ```bash
   # 在主菜单中选择 source
   # 选择 add 添加新连接
   # 输入数据库连接信息：
   #   - 数据库类型：MySQL / PostgreSQL / SeekDB
   #   - 主机地址
   #   - 端口
   #   - 数据库名
   #   - 用户名和密码
   ```

4. **开始查询**
   ```bash
   # 在主菜单中选择 chat
   # 选择数据源
   # 输入自然语言问题，例如：
   #   "显示最近一周的销售额"
   #   "统计每个类别的商品数量"
   #   "查看用户注册趋势"
   ```

## 💡 功能特性

### 1. 自然语言转 SQL

无需编写 SQL，直接用自然语言提问：

```
aiq> 显示最近一周的订单总数
```

AIQ 会自动：
- 理解你的问题
- 生成对应的 SQL 查询
- 执行查询并返回结果

### 2. 图表可视化

查询结果可以自动可视化为多种图表类型：

#### 📊 柱状图 (Bar Chart)
适用于分类数据对比，例如：
```sql
SELECT category, COUNT(*) AS count FROM products GROUP BY category;
```

#### 📈 折线图 (Line Chart)
适用于时间序列数据，例如：
```sql
SELECT date, SUM(amount) AS total FROM sales GROUP BY date ORDER BY date;
```

#### 🥧 饼图 (Pie Chart)
适用于比例分布，例如：
```sql
SELECT status, COUNT(*) AS count FROM orders GROUP BY status;
```

#### 📉 散点图 (Scatter Plot)
适用于数值关系分析，例如：
```sql
SELECT price, sales FROM products;
```

### 3. 智能图表检测

AIQ 会根据查询结果的数据结构自动检测最适合的图表类型：
- **分类 + 数值** → 柱状图或饼图
- **时间 + 数值** → 折线图
- **数值 + 数值** → 散点图

### 4. 多数据库支持

支持多种数据库类型：
- **MySQL** - 最常用的关系型数据库
- **PostgreSQL** - 功能强大的开源数据库
- **SeekDB** - OceanBase 分布式数据库

### 5. 交互式界面

- 🎯 清晰的菜单导航
- 🎨 彩色输出和语法高亮
- ⌨️ 支持中文输入
- 📋 表格和图表展示

## 📚 使用指南

### 主菜单

AIQ 提供简洁的主菜单：

```
AIQ - Main Menu
? config   - Manage LLM configuration
  source   - Manage database connections
  chat     - Query database with natural language
  exit     - Exit application
```

### 配置管理

在主菜单中选择 `config` 可以：
- 查看当前 LLM 配置
- 更新 API URL
- 更新 API Key
- 更新模型名称

### 数据源管理

在主菜单中选择 `source` 可以：
- 添加新的数据库连接
- 查看所有已配置的数据源
- 删除数据源

### 查询模式

在主菜单中选择 `chat` 进入查询模式：

1. **选择数据源** - 从已配置的数据源中选择一个
2. **输入问题** - 用自然语言描述你的查询需求
3. **查看结果** - 选择以表格、图表或两者同时显示
4. **选择图表类型** - 如果有多种图表类型可选，选择你喜欢的

### 示例查询

```
# 统计查询
aiq> 统计每个部门的员工数量

# 趋势分析
aiq> 显示最近一个月的销售趋势

# 分布分析
aiq> 查看订单状态的分布情况

# 关联分析
aiq> 分析商品价格和销量的关系
```

## ⚙️ 配置

配置文件存储在 `~/.aiqconfig/`：

- **config.yaml** - LLM 配置（URL、API Key、模型）
- **sources.yaml** - 数据库连接配置

### 配置示例

**config.yaml**
```yaml
llm:
  url: https://api.openai.com/v1
  apiKey: sk-...
  model: gpt-4
```

**sources.yaml**
```yaml
sources:
  - name: local-mysql
    type: MySQL
    host: localhost
    port: 3306
    database: testdb
    username: root
    password: password
```

## 🛠️ 开发

### 项目结构

```
aiq/
├── cmd/aiq/          # 主程序入口
├── internal/
│   ├── cli/          # CLI 命令和菜单系统
│   ├── config/       # 配置管理
│   ├── source/       # 数据源管理
│   ├── sql/          # SQL 交互模式（chat 模式）
│   ├── llm/          # LLM 客户端集成
│   ├── db/           # 数据库连接和查询执行
│   ├── chart/        # 图表可视化（柱状图、折线图、饼图、散点图）
│   └── ui/           # UI 组件（提示、颜色、加载、图表）
└── openspec/         # OpenSpec 变更管理
```

### 构建

```bash
go build -o aiq cmd/aiq/main.go
```

### 运行测试

```bash
go test ./...
```

## 📝 许可证

本项目采用 [Apache License 2.0](LICENSE) 许可证。

## 🤝 贡献

欢迎贡献！请随时提交 Pull Request。

---

<div align="center">

**Made with ❤️ using Go**

[报告问题](https://github.com/aiq/aiq/issues) · [提交功能请求](https://github.com/aiq/aiq/issues) · [查看文档](https://github.com/aiq/aiq)

</div>
